            ...
            {messages.map((msg, idx) => {
              ...
              return (
                <div key={msg.id || idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group hover:-translate-y-0.5 transition-transform duration-200`}>
                  ...
                  <div
                    className={`max-w-[70%] px-5 py-3.5 rounded-3xl backdrop-blur-md relative text-sm md:text-base leading-relaxed ${isMe
          ...
          <div className="p-4 bg-black/20 backdrop-blur-xl border-t border-white/10 z-20">
            <div className="max-w-4xl mx-auto relative flex items-end space-x-2 bg-white/5 border border-white/10 rounded-3xl p-2 transition-all focus-within:bg-white/10 focus-within:border-indigo-500/50 shadow-inner">
              ...
              <button
                ...
                className={`p-3 rounded-full flex items-center justify-center transition-all duration-300 ${inputValue.trim() && isConnected
